...
#    • мягкий мерж .env (дописать недостающие ключи, не трогая значения);
...
merge_config(){
  ...
  if [ "$added" -gt 0 ]; then ok ".env дополнен новыми ключами: ${added}"
  else info ".env актуален — новых ключей нет"; fi
}
